...
static void max_heapify_heap(heap *h, int id_p);
...
int indice_filho_esq(heap *h, int id_p) { // id_p: índice de um pai
    int esq = 2*id_p + 1;
    if(esq >= h->qtde)
        return FLAG_ERRO;
...
int indice_filho_dir(heap *h, int id_p) { // id_p: índice de um pai
    int dir = 2*id_p + 2;
    if (dir >= h->qtde)
        return FLAG_ERRO;
...
void max_heapify(heap *h, int id_p) {          // id_p: índice do elemento pai
